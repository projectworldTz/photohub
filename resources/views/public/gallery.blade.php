            ...
            @foreach($photos as $p)
                ...
                <div class="col-6 col-md-4">
                    <article class="content-card h-100 p-2 proof-card {{ $selectedPhotoIds->contains($p->id)?'border border-danger border-3':'' }}">
                        <img class="w-100 rounded gallery-image" role="button" loading="lazy" src="{{ route('public.gallery.photo', [$gallery->code, $p]) }}" alt="{{ $p->filename }}" data-bs-toggle="modal" data-bs-target="#lightbox">
                        <div class="d-flex flex-wrap gap-1 mt-2">
                            ...
                            @if($canDownload)<a class="btn btn-success btn-sm" href="{{ route('public.gallery.download', [$gallery->code, $p]) }}">Download</a>@endif
                        </div>
                    </article>
                </div>
            @endforeach
...
const galleryImages=[...document.querySelectorAll('.gallery-image')]; let current=0; const lightbox=document.getElementById('lightbox-image'); galleryImages.forEach((img,index)=>img.addEventListener('click',()=>{current=index;lightbox.src=img.src})); document.getElementById('previous').onclick=()=>{current=(current-1+galleryImages.length)%galleryImages.length;lightbox.src=galleryImages[current].src}; document.getElementById('next').onclick=()=>{current=(current+1)%galleryImages.length;lightbox.src=galleryImages[current].src};
</script>
@endpush
