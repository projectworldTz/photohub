...
<div class="row g-3">@forelse($galleries as $gallery)<div class="col-sm-6 col-xl-4"><div class="content-card h-100"><span class="badge text-bg-light">{{ str($gallery->type)->replace('_',' ')->title() }}</span><h4 class="mt-3">{{ $gallery->name }}</h4><p>{{ $gallery->photos_count }} photos · {{ str($gallery->status)->replace('_',' ')->title() }}</p><small class="d-block">{{ $gallery->customer?->full_name ?: 'No customer' }}</small><a class="btn btn-sm btn-light mt-3" href="{{ route('galleries.show',$gallery) }}">Manage gallery</a></div></div>@empty<div class="col-12"><div class="content-card text-center text-muted">No galleries match these filters.</div></div>@endforelse</div>
{{ $galleries->links() }}
@endsection
